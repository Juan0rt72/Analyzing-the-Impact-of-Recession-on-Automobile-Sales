...
@app.callback(
    ...
    Input(component_id='drop-statistics',component_property='value'))
...
def update_output_container(sel_year, seL_stat):
    if seL_stat == 'Recession Period Statistics':
...
# TASK 2.6: Create and display graphs for Yearly Report Statistics
 # Yearly Statistic Report Plots                             
    elif (sel_year and seL_stat=='Yearly Statistics') :
        yearly_data = data[data['Year'] == sel_year]
...
# Plot 2 Total Monthly Automobile sales using line chart.
        ...
        Y_chart3 = dcc.Graph( figure= px.bar(avr_vdata, x = 'Vehicle_Type', y = 'Automobile_Sales',
                                             title='Average Vehicles Sold by Vehicle Type in the year {}'.format(sel_year)))

            # Total Advertisement Expenditure for each vehicle using pie chart
        exp_data=yearly_data.groupby('Vehicle_Type')['Advertising_Expenditure'].sum().reset_index()
        Y_chart4 = dcc.Graph(figure =px.pie( exp_data, values = 'Advertising_Expenditure', names= 'Vehicle_Type',
                          title = 'Total Advertisement expenditure share by vehicle in the year {}'.format(sel_year)))
